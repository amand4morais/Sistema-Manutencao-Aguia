...
import {
  LayoutDashboard, HardHat, User, LogOut,
  Menu, X, Shield, Wrench, FileText, Calendar
} from 'lucide-react';
...
export function Layout() {
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex flex-col md:flex-row">
      ...
      {/* ── Header mobile ─────────────────────────────────────── */}
      <header className="md:hidden bg-white border-b border-stone-200 px-4 flex items-center justify-between sticky top-0 z-50 h-16 shrink-0">
        <span className="font-bold text-emerald-800 text-base">Águia Florestal</span>
      ...
      {/* ── Sidebar ───────────────────────────────────────────── */}
      <aside className={cn(
        // Mobile: fixed, começa ABAIXO do header (top-16 = 64px)
        // Desktop: relative, ocupa altura total
        'fixed top-16 bottom-0 left-0 z-40 w-64 bg-white border-r border-stone-200',
        'transform transition-transform duration-300 ease-in-out',
        'md:relative md:top-0 md:bottom-auto md:translate-x-0 md:flex md:flex-col md:h-screen md:sticky md:top-0',
        ...
        {/* Nav com scroll próprio para não cortar itens */}
        <nav className="flex-1 overflow-y-auto px-4 py-4 md:py-0 space-y-1">
          <p className="px-4 text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-2">
            Menu Principal
          </p>
          {menuItems.map((item) => {
          ...
          {profile?.role === 'admin' && (
            <>
              <p className="px-4 text-[10px] font-bold text-stone-400 uppercase tracking-widest mt-6 mb-2">
                Administração
              </p>
              {adminItems.map((item) => {
                const Icon     = item.icon;
      ...
      <main className="flex-1 overflow-auto flex flex-col min-w-0">
        ...
        {/* Header desktop */}
        <header className="hidden md:flex bg-white border-b border-stone-200 px-8 py-4 items-center justify-between sticky top-0 z-50 shrink-0">
          <h2 className="text-sm font-medium text-stone-600">
            {[...menuItems, ...adminItems].find(item => item.path === location.pathname)?.label
