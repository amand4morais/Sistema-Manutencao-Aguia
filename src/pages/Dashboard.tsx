  ...
  const { data: dashboardData, isLoading: loading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const chartData = last7Days.map(date => {
        ...
        const dayMaintenances = completedLast7Days?.filter(m => {
        ...
        return {
          name: dayName,
          concluida: dayMaintenances.filter(m => m.status === 'completed').length,
          pendente: dayMaintenances.filter(m => m.status === 'pending').length
        };
      });
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard 
          ...
          label="Prev. Pendentes"
          value={stats.pendingPreventive}
          color="bg-amber-50"
          trend="Aguardando"
        />
        <StatCard 
          icon={<AlertTriangle className="text-red-600" />}
          label="Corr. Pendentes"
          value={stats.pendingCorrective}
          color="bg-red-50"
          trend="Urgente"
        />
        <StatCard 
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  <Bar dataKey="concluida" name="Concluída" fill="#059669" radius={[4, 4, 0, 0]} barSize={30} />
                  <Bar dataKey="pendente" name="Pendente" fill="#fbbf24" radius={[4, 4, 0, 0]} barSize={30} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-stone-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                return (
                  <div key={i} className={`p-4 rounded-xl border ${isOverdue ? 'border-red-100 bg-red-50/50' : 'border-stone-100 bg-stone-50/50'}`}>
                    <div className="flex justify-between items-start mb-2">
                      <p className="font-bold text-stone-900 truncate pr-2">{item.equipmentName}</p>
                      <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${isOverdue ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>
              ...
              {upcomingPreventives.length === 0 && (
                <p className="col-span-2 text-center text-stone-400 py-4 text-sm italic">Nenhuma preventiva próxima</p>
              )}
            </div>
          </div>
        </div>
            ...
            {recentMaintenances.length > 0 ? (
              recentMaintenances.map((m, i) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-xl hover:bg-stone-50 transition-colors border border-transparent hover:border-stone-100">
                  <div className={`mt-1 w-2 h-2 rounded-full ${m.type === 'Preventiva' ? 'bg-amber-500' : 'bg-red-500'}`} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-stone-900 truncate">
                      {m.equipments?.name || 'Equipamento'}
                    </p>
                    <p className="text-xs text-stone-500">
                      {m.type} • {format(new Date(m.created_at), "dd 'de' MMM", { locale: ptBR })}
                    </p>
                  </div>
                  <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${
                    m.status === 'completed' ? 'bg-emerald-50 text-emerald-700' : 
                    m.status === 'in_progress' ? 'bg-blue-50 text-blue-700' : 'bg-stone-100 text-stone-600'
                  }`}>
