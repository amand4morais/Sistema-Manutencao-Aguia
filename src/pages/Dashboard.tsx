    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {/* Botão restrito a admins — funcionários não veem */}
          {profile?.role === 'admin' && (
            <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-6 rounded-2xl border border-stone-200 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-bold text-stone-800 flex items-center gap-2">
                <Calendar size={19} className="text-amber-600" />
                Próximas Preventivas
              </h2>
              <Link
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 rounded-xl border ${
                        isOverdue ? 'border-red-100 bg-red-50/50' : 'border-stone-100 bg-stone-50/50'
                      }`}
                    >
          ...
          <div className="space-y-3">
            {recentMaintenances.length > 0 ? (
              recentMaintenances.map((m, i) => (
                <div
                  key={i}
                  className="flex items-start gap-3 p-3 rounded-xl hover:bg-stone-50 transition-colors border border-transparent hover:border-stone-100"
                >
                  <div className={`mt-1.5 w-2 h-2 rounded-full shrink-0 ${
                    m.type === 'Preventiva' ? 'bg-amber-500' : 'bg-red-500'
                  }`} />
